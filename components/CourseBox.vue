...
<template>
    <div class="bg-white rounded grap-10 overflow-hidden shadow-card">
        ...
        <div class="mt-7 flex justify-between items-center gap-4 mb-4">
            <NuxtLink :to="`/${course.id}`">
                <a href="single" class="text-lg-primary w-full hover:underline">{{course.name}}</a>
            </NuxtLink>
        </div>
        <div>
            <div v-for="author in course.authors" :key="author.id" class="flex items-center mb-2">
                <div class="flex items-center">
                    <img src="https://laravel-courses.com/storage/thumbnails/default/56/tp/g7t7058gcgwk00s48cwww.png?p=authors%2Fa489f89a-4866-4c10-961f-5fa6526d9709.png&amp;s=pd&amp;smartcrop=28x28" :alt="author.name" class="w-7 h-7 object-contain flex shrink-0 rounded-full">
                </div>
                <h4 class="text-sm-primary w-full leading-5 ml-2"> {{author.name}} </h4>
            </div>
            <div class="mt-3">
            <div class="flex items-center gap-3 mt-6">
                ...
                </svg> {{showDuration(course.duration)}}
                </p>
                ...
                </svg> {{showLevel(course.difficulty_level)}}
                </p>
            </div>
            </div>
        </div>
